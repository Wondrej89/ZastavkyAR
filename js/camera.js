...
export class CameraController {
 constructor(video,mediaDevices=navigator.mediaDevices){this.video=video;this.mediaDevices=mediaDevices;this.stream=null;this.cameraStarting=false;this.wantsCamera=false;this.lifecycleVersion=0;this.startingPromise=null;this.getUserMediaAttempted=false;this.lastError=null;}
 ...
 start(){
  this.getUserMediaAttempted=true;this.lastError=null;
 ...
 async startUntilCurrent(){
  try{
   while(this.wantsCamera){
    ...
    let stream;
    try{stream=await this.mediaDevices.getUserMedia(CAMERA_CONSTRAINTS)}catch(error){this.lastError=error;if(this.wantsCamera&&version!==this.lifecycleVersion)continue;throw error}
