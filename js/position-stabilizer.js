...
export class PositionStabilizer {
  ...
  reset(){this.started=this.now();this.samples=[];this.pending=[];this.largePending=[];this.anchor=null;this.lastUpdate=-Infinity;this.lastRawPositionAt=null;this.lastAcceptedPositionAt=null;this.anchorStale=false}
  markAnchorStale(){this.anchorStale=true;this.pending=[];this.largePending=[]}
  reanchor(fix,reason='manual'){
    this.samples=[];this.pending=[];this.largePending=[];this.anchorStale=false;this.hardReanchorCount++;this.lastHardReanchorReason=reason;this.publish({...fix},true);this.onHardReanchor(this.anchor,reason);return 'reanchored';
  }
  add(fix,{resume=false}={}){
    ...
    if(!this.anchor){
      ...
      this.publish(this.average(stable?candidates:(quality.length?quality:[fix])));return 'accepted';
    }
  ...
  publish(position,force=false){if(!force&&this.now()-this.lastUpdate<this.config.gpsAnchorMinIntervalMs)return;this.anchor=position;this.lastUpdate=this.now();this.lastAcceptedPositionAt=this.lastUpdate;this.onPosition(this.anchor)}
  diagnostics(){return{lastRawPositionAt:this.lastRawPositionAt,lastAcceptedPositionAt:this.lastAcceptedPositionAt,movementMode:this.movementMode||'static',hardReanchorCount:this.hardReanchorCount,lastHardReanchorReason:this.lastHardReanchorReason,anchorStale:this.anchorStale}}
