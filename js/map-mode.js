...
export class MapMode {
  ...
  async show(initialPosition = this.state.rawPosition || this.state.position) {
    ...
    catch (error) { this.lastError = { message: error?.message ?? String(error ?? ''), status: error?.status ?? null }; this.showError(error); this.onDiagnosticsChange(); }
  }
  ...
  async ensureMap(initialPosition = this.state.rawPosition || this.state.position) {
    ...
    this.initializing = this.loadLibrary().then(maplibregl => {
      ...
      this.map.on('error', event => { this.lastError = { message: event?.error?.message ?? String(event?.error ?? ''), status: event?.error?.status ?? null }; this.onDiagnosticsChange(); });
      this.map.on('load', () => { this.mapLoaded = true; this.error.classList.add('hidden'); this.lastError = null; if (this.state.viewMode !== 'ar') { this.updatePosition(initialPosition, true); this.activateGeolocation(); } this.onDiagnosticsChange(); });
      return this.map;
