...
test('a raw GPS update only one metre away still recenters an initialized map',()=>{const center=[];const state={rawPosition:{latitude:50,longitude:14},heading:null,mapRotationMode:'heading-up'};const root={style:{setProperty(){}},classList:{toggle(){}}},compass={dataset:{},setAttribute(){}};const mode=new MapMode({root,container:{},error:{},compass,config:{mapMarkerRefreshMeters:18},state,select(){}});mode.map={easeTo:v=>center.push(v.center),setBearing(){}};mode.lastMarkerPosition={...state.rawPosition};mode.updatePosition();state.rawPosition={latitude:50+1/111111,longitude:14};mode.updatePosition();assert.deepEqual(center.at(-1),[14,50+1/111111])});
