...
test('visual effect is repeatable, cancellable and fixed at 180 ms',async()=>{const source=await readFile(new URL('../js/app.js',import.meta.url),'utf8');const transition=source.match(/function playViewTransition\(direction\).*?\nconst viewModeController/s)?.[0]??'';assert.match(transition,/getAnimations\?\.\(\)\.forEach\(animation=>animation\.cancel\(\)\)/);assert.match(transition,/duration:180/);assert.doesNotMatch(transition,/mapMode|mapLoaded|state\.viewMode/)});
