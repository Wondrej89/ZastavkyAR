...
test('app completes mode lifecycle before starting the independent visual effect',async()=>{const source=await readFile(new URL('../js/app.js',import.meta.url),'utf8');const enter=source.match(/function enterMapMode\(\).*?\nfunction exitMapMode/s)?.[0]??'';const exit=source.match(/function exitMapMode\(\).*?\nfunction playViewTransition/s)?.[0]??'';assert.ok(enter.indexOf("state.viewMode='map'")<enter.indexOf("playViewTransition('to-map'"));assert.ok(enter.indexOf('ensureLocationTracking().stop()')<enter.indexOf("playViewTransition('to-map'"));assert.ok(enter.indexOf('mapMode.show(initialPosition)')<enter.indexOf("playViewTransition('to-map'"));assert.ok(exit.indexOf("state.viewMode='ar'")<exit.indexOf("playViewTransition('to-ar'"));assert.ok(exit.indexOf('mapMode.deactivateGeolocation()')<exit.indexOf("playViewTransition('to-ar'"));assert.ok(exit.indexOf('mapMode.hide()')<exit.indexOf("playViewTransition('to-ar'"));assert.ok(exit.indexOf('tracking.restartLocationTracking()')<exit.indexOf("playViewTransition('to-ar'"))});

test('visual effect is repeatable, cancellable, directional and independent',async()=>{const source=await readFile(new URL('../js/app.js',import.meta.url),'utf8');const transition=source.match(/function playViewTransition\(direction\).*?\nconst viewModeController/s)?.[0]??'';assert.match(transition,/getAnimations\?\.\(\{subtree:true\}\)\.forEach\(animation=>animation\.cancel\(\)\)/);assert.match(transition,/duration:240/);assert.match(transition,/duration:120/);assert.match(transition,/prefers-reduced-motion: reduce/);assert.match(transition,/rotateX/);assert.doesNotMatch(transition,/mapMode|mapLoaded|state\.viewMode/)});

test('transition markup and styles isolate the 3D effect from map and camera content',async()=>{const [html,css]=await Promise.all([readFile(new URL('../index.html',import.meta.url),'utf8'),readFile(new URL('../styles.css',import.meta.url),'utf8')]);assert.match(html,/transition-half transition-top/);assert.match(html,/transition-half transition-bottom/);assert.match(html,/transition-horizon/);assert.match(css,/\.view-transition-overlay\{[^}]*pointer-events:none;[^}]*perspective:900px/);assert.match(css,/\.transition-top\{[^}]*transform-origin:bottom center/);assert.match(css,/\.transition-bottom\{[^}]*transform-origin:top center/);assert.doesNotMatch(css,/(?:#camera|#map(?:-view)?|\.maplibregl-(?:map|canvas))[^{}]*\{[^}]*\b(?:perspective|rotateX|scale)\b/)});
...
test('resource errors stay diagnostic and a successful load clears stale map errors',async()=>{
  const listeners={};
  class Map { on(name,callback){listeners[name]=callback} }
  const classes=new Set(['hidden']);
  const error={textContent:'',classList:{add:value=>classes.add(value),remove:value=>classes.delete(value)}};
  const state={viewMode:'ar',rawPosition:null,heading:null,mapRotationMode:'north-up'},root={style:{setProperty(){}},classList:{toggle(){}}},compass={dataset:{},setAttribute(){}};
  const mode=new MapMode({root,container:{},error,compass,config:{mapyApiKey:'key',mapZoom:16.5},state,select(){},loadLibrary:async()=>({Map})});
  await mode.ensureMap();
  listeners.error({error:new Error('one tile failed')});
  assert.equal(mode.lastError.message,'one tile failed');assert.equal(classes.has('hidden'),true);
  listeners.load();
  assert.equal(mode.mapLoaded,true);assert.equal(mode.lastError,null);assert.equal(classes.has('hidden'),true);
  mode.showError(new Error('stale failure'));
