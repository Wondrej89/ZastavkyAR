...
test('stop clears the custom watch without discarding tracking state',()=>{const cleared=[],geo={watchPosition(){return 7},clearWatch(id){cleared.push(id)}};const tracking=new LocationTracking({geolocation:geo,config:CONFIG,onFix:()=>{}});tracking.restartLocationTracking();tracking.stop();assert.deepEqual(cleared,[7]);assert.equal(tracking.watch,null);tracking.stop();assert.deepEqual(cleared,[7])});

test('map-to-ar seed publishes exact fix immediately and next AR fix uses stabilization',()=>{let now=10000;const values=[],stabilizer=new PositionStabilizer({...CONFIG,gpsAnchorSampleCount:7,gpsAnchorMinIntervalMs:0},position=>values.push(position),()=>now);const mapFix=fix(25,{accuracy:6,timestamp:now});assert.equal(stabilizer.seedFromFix(mapFix,'map-to-ar'),'reanchored');assert.equal(values.length,1);assert.deepEqual(values[0],mapFix);assert.equal(stabilizer.diagnostics().anchorSource,'map-to-ar');now++;assert.equal(stabilizer.add(fix(26,{accuracy:6,timestamp:now})),'ignored');assert.equal(values.length,1)});

test('fresh position request is high accuracy and bypasses cache',async()=>{let success,requested;const geo={getCurrentPosition(callback,error,options){success=callback;requested=options}};const tracking=new LocationTracking({geolocation:geo,config:CONFIG,onFix:()=>{}});const pending=tracking.requestFreshPosition();success({coords:{latitude:50,longitude:14,accuracy:5,speed:null},timestamp:123});assert.deepEqual(await pending,{latitude:50,longitude:14,accuracy:5,speed:null,timestamp:123});assert.deepEqual(requested,{enableHighAccuracy:true,maximumAge:0,timeout:10000})});
