export function buildMarkers({ stops, routes, trips, stopTimes }) {
  ...
  for (const time of stopTimes) {
    const trip = tripById.get(time.trip_id), route = trip && routeById.get(trip.route_id);
    if (!route) continue;
    if (!usage.has(time.stop_id)) usage.set(time.stop_id, new Map());
    const key = Number(route.route_type);
    if (!usage.get(time.stop_id).has(key)) usage.get(time.stop_id).set(key, new Set());
    usage.get(time.stop_id).get(key).add(route.route_short_name || route.route_long_name || '');
  }
  ...
  for (const stop of stops) {
    ...
    if (modes.includes(2) && modes.filter(mode => mode !== 2).length === 0) add(stop, { kind:'train_station', stationId:stop.parent_station || stop.stop_id, modes:[2] });
    else add(stop, { kind:'surface', modes });
  }
  for (const station of stops.filter(stop => Number(stop.location_type || 0) === 1)) {
    ...
    if (metroStops.length) {
      ...
      const railStops = kids.filter(stop => usage.get(stop.stop_id)?.has(2));
      if (railStops.length) add(station, { kind:'train_station', stationId:station.stop_id, departureStopIds:railStops.map(s => s.stop_id).sort(), modes:[2] });
    }
  }
